...
import type { Team, UserProfile, Tournament, TeamApplication } from '@/lib/types';
import {
  Card,
  CardContent,
  CardDescription,
...
export default function TeamProfilePage() {
  ...
  const [applicationStatus, setApplicationStatus] = useState<'idle' | 'pending' | 'loading'>('loading');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const pendingApp = snapshot.docs.find(doc => doc.data().status === 'pending');
      if (pendingApp) {
    ...
    startTransition(async () => {
        ...
        if (result.success) {
            toast({
                ...
                description: 'Tu solicitud ha sido enviada al líder del equipo para su revisión.',
            });
            setApplicationStatus('pending');
        } else {
            toast({
  ...
  const canApply = team.lookingForPlayers && team.founder !== user?.uid && !userProfile?.teamId;
                        ...
                        {canApply && (
                            applicationStatus === 'idle' ? (
                                ...
                                <Button variant="outline" disabled>
                                    <Clock className="mr-2 h-4 w-4" /> Solicitud Pendiente
                                </Button>
                            ) : (
                                <Button disabled><Skeleton className="h-5 w-32" /></Button>
                            )
                        )}
                    </CardHeader>
