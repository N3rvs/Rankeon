...
export const updateUserPresence = onCall(async ({ auth: callerAuth, data }) => {
    if (!callerAuth) {
        throw new HttpsError('unauthenticated', 'You must be logged in to update your status.');
    }
    const { uid } = callerAuth;
    const { status } = data as { status: 'available' | 'busy' | 'away' };
