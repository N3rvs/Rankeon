...
export const deleteTeamV2 = onCall(async ({ auth, data }: { auth?: any, data: DeleteTeamData }) => {
    ...
    try {
        ...
        // Get the user's current role to determine if they should be demoted.
        const userToUpdate = await admin.auth().getUser(uid);
        const existingClaims = userToUpdate.customClaims || {};
        const shouldChangeRole = existingClaims.role === 'founder';

        // Proceed with deletion using an atomic batch write.
        ...
        // 3. CONDITIONALLY update the user's role in their Firestore document.
        if (shouldChangeRole) {
        ...
        // After the batch succeeds, CONDITIONALLY update the custom auth claims.
        if (shouldChangeRole) {
             await admin.auth().setCustomUserClaims(uid, { ...existingClaims, role: "player" });
        }
