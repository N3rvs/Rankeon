...
export const updateUserRole = onCall(async ({ auth: callerAuth, data }) => {
...
export const updateUserStatus = onCall(async ({ auth: callerAuth, data }) => {
  ...
  try {
    ...
    let banUntil = null;
    if (disabled && duration) { // Temporary ban
...
export const updateUserCertification = onCall(async ({ auth: callerAuth, data }) => {
  checkModOrAdmin(callerAuth);
