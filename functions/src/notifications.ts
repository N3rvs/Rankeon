...
function assertAuth(ctx: any) { if (!ctx.auth) throw new HttpsError('unauthenticated', 'You must be logged in.'); return ctx.auth.uid; }

const AddNotifSchema = z.object({ to: z.string().min(1), type: z.enum(notifTypes), extraData: z.record(z.any()).optional() });

export const addInboxNotification = onCall({ region: 'europe-west1' }, async (req: any) => {
...
export const markNotificationsAsRead = onCall({ region: 'europe-west1' }, async (req: any) => {
    const uid = assertAuth(req);
...
export const deleteNotifications = onCall({ region: 'europe-west1' }, async (req: any) => {
    const uid = assertAuth(req);
    const { ids } = z.object({ ids: z.array(z.string().min(1)).min(1).max(100) }).parse(req.data);
    const snaps = await db.getAll(...ids.map(id => db.doc(`notifications/${id}`)));
    const batch = db.batch();
...
export const getInbox = onCall({ region: 'europe-west1' }, async (req: any) => {
    const uid = assertAuth(req);
