...
async function assertAreFriends(a: string, b: string) {
  ...
  if (!fs.exists)
    throw new HttpsError('permission-denied', 'You can only DM friends.');
}
...
export const sendMessageToFriend = onCall(
  { region: 'europe-west1' },
  async (req: any) => {
    const uid = assertAuth(req);
    const { to, text, clientId } = MessageSchema.parse(req.data);
    if (uid === to)
      throw new HttpsError('failed-precondition', 'Cannot DM yourself.');
...
export const getChats = onCall({ region: 'europe-west1' }, async (req: any) => {
  const uid = assertAuth(req);
...
export const getChatMessages = onCall(
  ...
  async (req: any) => {
    ...
    const members = (chat.data() as any)?.members ?? [];
...
export const deleteChatHistory = onCall(
  { region: 'europe-west1' },
  async (req: any) => {
    const uid = assertAuth(req);
    ...
    await chatRef.set(
      {
        ...
        lastMessageAt: Timestamp.now(),
        unread: members.reduce(
          (acc: any, m: any) => ((acc[m] = 0), acc),
          {}
        ),
        lastReadAt: members.reduce(
          (acc: any, m: any) => ((acc[m] = Timestamp.now()), acc),
          {}
        ),
      },
      { merge: true }
...
export const blockUser = onCall({ region: 'europe-west1' }, async (req: any) => {
  const uid = assertAuth(req);
...
export const unblockUser = onCall({ region: 'europe-west1' }, async (req: any) => {
  const uid = assertAuth(req);
  const { userId } = BlockSchema.parse(req.data ?? {});
  const ref = db.doc(`blocks/${uid}/targets/${userId}`);
  await ref.delete();
